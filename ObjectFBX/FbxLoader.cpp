...
const std::string FbxLoader::defaultTexTureFiname = "white1x1.png";
...
void FbxLoader::ParseNodeRecursive(FbxModel* model, FbxNode* fbxNode, Node* parent)
{
	...
	//ローカル変形行列の計算
	node.transfrom = XMMatrixIdentity();
	node.transfrom *= matScaling;			//ワールド行列にスケーリングを反映
	node.transfrom *= matRotation;			//ワールド行列に回転を反映
	node.globalTransform *= matTranslation;	//ワールド行列に平行移動を反映

	//グローバル変形行列の計算
	node.globalTransform = node.transfrom;
	if (parent) {
		node.parent = parent;
		//親の変形を乗算
	...
	for (int i = 0; i < polygonCount; i++) {
		...
		for (int j = 0; j < polygonSize; j++) {
			...
			//4頂点目
				...
				//3点追加し、四角形の0,1,2,3内 2,3,0で三角形を構築する
...
void FbxLoader::ParseMaterial(FbxModel* model, FbxNode* fbxNode)
{
	...
	if (materialCount > 0) {
		...
		if (!textureLoaded) {
			LoadTexture(model, baseDirectory + defaultTexTureFiname);
		}
	}
}
...
void FbxLoader::ParseSkin(FbxModel* model, FbxMesh* fbxMesh)
{
	...
	if (fbxSkin == nullptr) {
		...
		for (int i = 0; i < model->vertices.size(); i++) {
			//最初のボーン(単位行列)の影響100%する
			model->vertices[i].boneIndex[0] = 0;
			model->vertices[i].boneWeight[0] = 1.0;
		}
		return;
	}
	...
	for (int i = 0; i < clusterCount; i++)
	{
		...
		//このボーンに影響を受ける頂点の配列
		int* controlPointIndeice = fbxCluster->GetControlPointIndices();
		double* constrolPointWeight = fbxCluster->GetControlPointWeights();
